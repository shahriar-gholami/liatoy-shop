...
class Product(models.Model):
    ...
    cost = models.IntegerField(default=0, verbose_name='قیمت خرید')
    ...
    def get_sales_price(self):
        if self.off_active == True and self.sales_price:
    ...
    def get_related_products(self):
        ...
        for cat in product_categoreis:
            if cat.is_sub == True:
                ...
                for product in products:
                    if product != self and product.price != 0:
...
class Order(models.Model):
    ...
    def get_final_payment(self):
        if self.delivery_method != None:
            if self.total_price >= self.delivery_method.min_cart_free and self.delivery_method.min_cart_free_active:
                total = self.total_price
            else:
                total = self.delivery_method.price + self.total_price
        else:
        ...
        if self.customer.wallet_balance != 0:
            if total >= self.customer.wallet_balance:
                ...
                total = 0
        return {'formated':f'{total:,}', 'unformated':total}
...
def thumbnail_upload_path(instance, filename):
    ...
    unique_name = f"{base_name}_{uuid.uuid4().hex}{ext.lower()}"
    return f"litoy_blog/{unique_name}"
...
class BlogPost(models.Model):
    ...
    author = models.CharField(max_length=255, default='لیاتوی')
    ...
    featured_image = models.ImageField(upload_to=thumbnail_upload_path, default='media/11.png', verbose_name='تصویر شاخص')
    created_date = models.DateTimeField(auto_now_add=True, verbose_name='تاریخ ایجاد')
    ...
    def get_faqs(self):
        return self.blogpostfaq_set.all()

    def get_meta_description(self):
        if self.meta_description:
            return self.meta_description
        return strip_tags(self.body)[:160]
